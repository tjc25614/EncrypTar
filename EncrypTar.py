...
def CreateTar(files, recursive):
    ...
    if os.path.isfile(files):
        ...
        os.chdir(files)
        tar_file = tar.open(mode='w:xz', fileobj=tar_buffer)
...
def RunEncrypTar():
    ...
    parser.add_argument("FILE", help='The file/directory to archive/(extract to), not recursive by default')
    ...
    if archive:
        if not os.path.isfile(args.FILE) and not os.path.isdir(args.FILE):
            raise ValueError("The file/directory must exist for it to be archived!")

        salt = os.urandom(SALT_LEN) # get a salt for key derivation
        key = DeriveKey(passphrase, salt)
        archive = CreateTar(args.FILE, recursive)
